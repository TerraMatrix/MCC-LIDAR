...
namespace mcc
{
	...
	class MCC_LIDAR_EXPORTS ProgressBar
	{
	public:
		typedef void (*progress_func)(float progress);
		...
		ProgressBar(progress_func func,
			int iterTimes);
		// Update the progress bar on the amount of work done so far.
		void updateProgress(double workDone);
		void updateWork(double workToBeDone);
