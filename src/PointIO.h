...
namespace mcc
{
	...
	MCC_LIDAR_FUNCTION_EXPORTS std::string  coordinateFmtSpec();

	// Formatter for a single point coordinate.
	extern boost::format coordinateFmt;

	// Convert coordinate to a string.
	MCC_LIDAR_FUNCTION_EXPORTS std::string  toStr(Coordinate coordinate);

	// Write a point as "(x, y, z)" to an output stream.
	MCC_LIDAR_FUNCTION_EXPORTS std::ostream& operator<<(std::ostream& stream,
