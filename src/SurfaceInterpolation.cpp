...
namespace mcc
{
  ...
  {
    return this->operator()(points, &useEveryPoint, cellResolution, tension, progress);
  }
                                                                     ...
                                                                     ProgressBar &progress)
  {
    ...
    rasterSurface_->setNoDataValue(CoordinateWithEnergy(-9999, 0.0));
    ...
    while (const IInterpolationRegion *region = regions->getNextRegion())
    {
      ...
      while (!splineComputed)
      {
        try
        {
          ...
          {
            (*rasterSurface_)[cell] = CoordinateWithEnergy(spline.interpolateHeight(cell.x(), cell.y()), spline.compute_bending_energy());
          }
        }
      ...
      nSplinesComputed++;
      progress.updateProgress(nSplinesComputed);
    }
    std::cout << std::endl;
