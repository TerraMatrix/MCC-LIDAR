...
namespace mcc
{
	...
	void Algorithm::classifyPoints(IUnclassifiedPoints& points,
		...
		double k,
		ProgressBar& progress, bool useModifiedVersion /*= true*/)
	{
		...
		const int maxPasses = useModifiedVersion ? 3 : 100; // max # of passes per scale domain
		int nDigitsForPass = (int)std::log10((double)maxPasses) + 1;
		std::string passFormat = boost::str(boost::format("0%d") % nDigitsForPass);

		const int maxIteration = useModifiedVersion ? 1 : 3;
		for (int SD = 1; SD <= maxIteration; SD++)
		{
			...
			do {
				...
				boost::shared_ptr<IRasterSurface> rasterSurface = surfaceInterpolation_(U, CR[SD], tension, progress);

				std::cout << indent << "Averaging raster surface..." << std::endl;

				if (useModifiedVersion) {
					rasterSurface->average(kCR[pass]); // kernel = 3x3
				}
				else {
				...
				{
					bool unground = 0;
					CoordinateWithEnergy surfaceHeight = (*rasterSurface)(point.x(), point.y());
					if (point.z() > surfaceHeight.coord + t[SD])
					{
						unground = 1;
						point.classifyAs(NonGround);
						if (nongroundPtsFile)
						{
							Coordinate canopyHeight = point.z() - surfaceHeight.coord;
