...
namespace mcc
{
  RasterSurface::RasterSurface(unsigned int rows,
                               ...
                               Coordinate cellSize)
      : Grid<CoordinateWithEnergy>(rows, columns, lowerLeft, cellSize, cellSize)
  {
    assert(rows > 0);
    assert(columns > 0);
    assert(cellSize > 0);
  ...
  void RasterSurface::average(int kernelSize)
  {
    ...
    boost::multi_array<CoordinateWithEnergy, 2> averages(boost::extents[kernelSize][columns()]);
    ...
    Grid<CoordinateWithEnergy> &grid = *this;
    ...
    BOOST_FOREACH (unsigned int row, topToBottom())
    {
      ...
      BOOST_FOREACH (unsigned int column, leftToRight())
      {
        CoordinateWithEnergy average(0, 0);
        ...
        else
        {
          ...
          average.coord = sum / count;
          CoordinateWithEnergy value;
          grid.getData(row, column, value);
          average.Energy = value.Energy;
        }
